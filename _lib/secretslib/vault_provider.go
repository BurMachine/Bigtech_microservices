...
// Set сохраняет секрет в Vault (для управления секретами)
func (p *VaultProvider) Set(ctx context.Context, key, value string) error {
	path := fmt.Sprintf("%s/data/%s", p.mountPath, p.secretPath)

	// Сначала читаем существующие секреты
	existing, err := p.GetAll(ctx)
	if err != nil && err.Error() != fmt.Sprintf("no secrets found at path: %s", path) {
	...
	// Добавляем/обновляем секрет
	existing[key] = value

	// Записываем обратно в Vault
...
// Delete удаляет секрет из Vault
...
// Keys возвращает список всех ключей в Vault по данному пути
func (p *VaultProvider) Keys(ctx context.Context) ([]string, error) {
	secrets, err := p.GetAll(ctx)
	if err != nil {
		return nil, err
	}
